 ...
 */
internal class TickerColumn(
    private val characterLists: ArrayList<TickerCharacterList>,
    private val metrics: TickerDrawMetrics
) {
    var currentChar = TickerUtils.EMPTY_CHAR
        private set

    private var targetChar = TickerUtils.EMPTY_CHAR
    ...
    private var currentCharacterList: CharArray? = null
    ...
    /**
     * Tells the column that the next character it should avatye_animator_cashfloter_show is {@param targetChar}. This can
     * change can either be animated or instant depending on the animation progress set by
     * [.setAnimationProgress].
     */
    fun setTargetChar(targetChar: Char) { // Set the current and target characters for the animation
        this.targetChar = targetChar
        sourceWidth = currentWidth
        targetWidth = metrics.getCharWidth(targetChar)
        minimumRequiredWidth = max(sourceWidth, targetWidth)
    ...
    private fun setCharacterIndices() {
        ...
        for (i in characterLists.indices) {
            val indices = characterLists[i].getCharacterIndices(
                currentChar,
                targetChar,
                metrics.preferredScrollingDirection
            )
            if (indices != null) {
                currentCharacterList = characterLists[i].characterList
    ...
    fun setAnimationProgress(animationProgress: Float) {
        if (animationProgress == 1f) { // Animation finished (or never started), set to stable state.
    ...
    fun draw(canvas: Canvas, textPaint: Paint) {
        ...
        drawText(
            canvas, textPaint, currentCharacterList, bottomCharIndex + 1,
        ...
        drawText(
            canvas, textPaint, currentCharacterList, bottomCharIndex - 1, bottomDelta + charHeight
        )
    }
    ...
    private fun drawText(
        canvas: Canvas, textPaint: Paint, characterList: CharArray?,
        index: Int, verticalOffset: Float
    ): Boolean {
